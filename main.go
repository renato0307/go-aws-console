...
func main() {
	...
	// Create an Amazon STS service clientSts
	clientSts := sts.New(session)

	getFedTokenInput := &sts.GetFederationTokenInput{
		Name:            aws.String("UserName"),
		DurationSeconds: aws.Int64(1800),
		Policy:          aws.String(`{"Version":"2012-10-17","Statement":{"Effect":"Allow","Action":"*","Resource":"*"}}`),
	}
	output, err := clientSts.GetFederationToken(getFedTokenInput)
	if err != nil {
		panic(err)
	}
